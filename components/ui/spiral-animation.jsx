...
// Brand accent (lime) for the spiral particles on a black field.
const PARTICLE_COLOR = '#c5ff00';
...
// Animation controller — GSAP-driven canvas spiral of particles.
class AnimationController {
  constructor(canvas, ctx, dpr, size) {
    ...
    // Create the stars once, under a seeded RNG for a stable, repeatable
    // spiral, then restore the real Math.random.
    this.createStarsSeeded();
    this.setupTimeline();
  }

  // Build all stars with a deterministic seed so the spiral looks identical
  // every time the transition plays. (Original code created stars twice and
  // half-applied the seed — fixed here to a single seeded pass.)
  createStarsSeeded() {
  ...
  setupTimeline() {
    // One-shot transition: a ~4s pass means the visible ~2.2s window reaches
    // well past the "change event" (t≈0.32) so the spiral actually blooms
    // before the overlay fades — the original 15s loop never got there.
...
export function SpiralAnimation() {
  ...
  // SSR-safe: start with zero dimensions, measure in the effect (no `window`
  // read during render).
